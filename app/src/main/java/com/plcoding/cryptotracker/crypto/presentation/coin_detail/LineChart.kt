...
fun LineChart(
    ...
    var xLabelWidthPx by remember {
        mutableFloatStateOf(0f)
    }
    LaunchedEffect(xLabelWidthPx) {
        onXLabelWidthChange(xLabelWidthPx)
    }
    ...
    Canvas(
        ...
        val minLabelSpacingYPx = chartStyle.minimumYLabelSpacing.toPx()
        ...
        xLabelWidthPx = maxXLabelWidth + xAxisLabelSpacingPx
            ...
            if (showHelperLines) {
                drawLine(
                    ...
                    strokeWidth = if (index == selectedDataPointIndex) {
                        chartStyle.helperLinesThicknessPx * 1.8f
                    } else {
        ...
        drawPoints = visibleDataPointsIndices.map {
            val x = viewPortLeftX + (it - visibleDataPointsIndices.first) *
                    xLabelWidthPx + (xLabelWidthPx / 2f)
